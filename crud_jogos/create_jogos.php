...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['titulo_jogo'], $_POST['data_jogo'], $_POST['descricao_jogo'], $_POST['armazernar_jogo']) && isset($_FILES['imagem_jogo'])) {
        ...
        if ($_FILES['imagem_jogo']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($imageFileType, $valid_types)) {
                if (move_uploaded_file($imagem["tmp_name"], $imagem_jogo)) {
                    ...
                    $query_check = "SELECT COUNT(*) FROM jogos WHERE titulo_jogo = :titulo_jogo";
                    $stmt_check = $conn->prepare($query_check);
                    $stmt_check->bindValue(':titulo_jogo', $titulo_jogo, PDO::PARAM_STR);
                    $stmt_check->execute();
                    $count = $stmt_check->fetchColumn();

                    if ($count == 0) {
                        // Inserir novo jogo no banco de dados
                        $query = "INSERT INTO jogos(titulo_jogo, imagem_jogo, data_jogo, descricao_jogo, iframe_url) VALUES(:titulo_jogo, :imagem_jogo, :data_jogo, :descricao_jogo, :iframe_url)";
                        try {
                            ...
                            if ($stmt->execute()) {
                                $sucesso_msg[] = 'Jogo cadastrado com sucesso!';
                            } else {
                                $error_msg[] = 'Erro ao cadastrar o jogo!';
                            }
    ...
    <div class="container">
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="titulo_jogo" id="titulo_jogo" placeholder="Informe Título do Jogo" required>
            </div>

            <div class="form-group">
                <label for="descricao_jogo">Descrição:</label>
                <textarea name="descricao_jogo" id="descricao_jogo" placeholder="Informe Descrição do Jogo" cols="30" rows="10" required></textarea>
            </div>

            <div class="form-group">
                <label for="imagem_jogo">Imagem:</label>
                <input type="file" name="imagem_jogo" id="imagem_jogo" required accept="image/jpg, image/jpeg">
            </div>

            <div class="form-group">
                <label for="data_jogo">Data do Jogo:</label>
                <input type="datetime-local" name="data_jogo" id="data_jogo" required>
            </div>

            <div class="form-group">
                <label for="armazernar_jogo">Código do iframe:</label>
                <textarea name="armazernar_jogo" id="armazernar_jogo" placeholder="Informe iframe do Jogo" cols="30" rows="3" required></textarea>
            </div>
